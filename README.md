...
`衣不缺` 是一个本地优先、图片优先的 HarmonyOS Stage 衣柜应用。它用于管理衣物、套装/美搭、穿着记录、逛店记录、心愿清单和统一搜索。

项目工程名保留为 `harmony-wardrobe`，应用包名为 `com.harmonywardrobe.app`。产品体验方向以 `衣不缺` 为准：白底、浅灰表面、黑色交互、真实照片优先，不做社交推荐流，不展示不存在的远程同步能力。
...
- 套装/美搭：记录穿搭照片和关联衣物，支持场景筛选、创建、编辑和删除。
- 逛店：记录店铺、商圈/地址、试穿备注、状态和图片，支持搜索、筛选、编辑和删除。
- 穿着记录：在套装页记录或编辑一次穿着，可关联套装并保留本地照片与备注。
- 我的：维护本地身形信息、偏好、隐私设置，并进入心愿清单。
- 心愿清单：记录想买的单品、门店、价格、备注和照片。
- 统一搜索：衣物、套装、逛店、店铺、心愿等内容进入可重建的搜索索引。
