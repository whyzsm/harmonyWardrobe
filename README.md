...
- 穿搭/美搭：记录穿搭照片和关联衣物，支持本地多选自定义分类筛选、创建、编辑和删除；未分类穿搭保留在“全部”中。
- 逛店：记录店铺、商圈/地址、试穿备注、状态和图片，支持搜索、筛选、编辑和删除。
- 穿着记录：在穿搭页记录或编辑一次穿着，可关联穿搭并保留本地照片与备注。
- 我的：维护本地身形信息、偏好、隐私设置，并进入心愿清单。
- 数据迁移：在“我的”中将本地 SQLite 数据和私有照片导出为单个备份文件；安装同一应用的新版本后可手动导入恢复，不依赖网络或云同步。
...
- 拍照/相册选择后的本地媒体复制。
- 导出备份、重新安装与导入后，记录数量、照片缩略图和详情图片均可恢复。
- 删除记录后的搜索索引和媒体清理。
